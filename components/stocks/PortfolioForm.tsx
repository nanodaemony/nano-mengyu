  ...
  return (
    <Card accent="stocks" elevation="sm">
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        ...
        <Input value={shares} onChange={(e) => setShares(e.target.value)} type="number" placeholder="100" label="数量" />
        <Input value={avgCost} onChange={(e) => setAvgCost(e.target.value)} type="number" step="0.01" placeholder="150.00" label="成本价" />
        <Button type="submit" className="col-span-2 sm:col-span-4 mt-1">保存</Button>
      </form>
    </Card>
  );
}
